...
class GameClientRunner:
    ...
    def __init__(self, session: RemoteSession, *, min_send_interval: float = 0.0) -> None:
        ...
        self._last_wire_send: float | None = None

    def _wire_send(self, line: str) -> None:
        """Send one protocol line, spacing consecutive sends by ``min_send_interval``."""
        if self._min_send_interval > 0 and self._last_wire_send is not None:
            wait = self._min_send_interval - (time.monotonic() - self._last_wire_send)
            if wait > 0:
                time.sleep(wait)
        self._session.send(line)
        if self._min_send_interval > 0:
            self._last_wire_send = time.monotonic()
    ...
    def execute(self, parts: list[str]) -> None:
        ...
        if command == "movemonsters":
            if len(args) != 1 or args[0] not in {"on", "off"}:
                raise InvalidCommand
            proto = shlex.join(["movemonsters", args[0]])
            self._wire_send(proto)
            return
        ...
        if command == "locale":
            if len(args) != 1:
                raise InvalidCommand
            proto = shlex.join(["locale", args[0]])
            self._wire_send(proto)
            return
...
def run_input_loop(runner: GameClientRunner, input_func=input) -> None:
    """Read user lines and forward them to ``runner`` until EOF."""
    while True:
        try:
            line = input_func()
        except EOFError:
...
class GameShell(cmd.Cmd):
    ...
    def do_movemonsters(self, arg: str) -> None:
        """Enable or disable random monster movement on server."""
        self._run("movemonsters", arg)

    def do_locale(self, arg: str) -> None:
        """Set preferred locale for server messages."""
        self._run("locale", arg)
...
def main() -> None:
    ...
    parser.add_argument(
        "--file",
        metavar="PATH",
        help="read commands from a file (.mood); disables interactive cmd input",
    )
    args = parser.parse_args()

    print(f"<<< Welcome to MOOD {VERSION} >>>")
    session = RemoteSession(args.host, args.port, args.username)
    script_mode = args.file is not None

    def reader_loop_interactive() -> None:
        while session.alive:
            try:
    ...
    reader = threading.Thread(
        target=reader_loop_script if script_mode else reader_loop_interactive,
        daemon=True,
    )
    reader.start()

    try:
        runner = GameClientRunner(
            session,
            min_send_interval=1.0 if script_mode else 0.0,
        )
        if script_mode:
