...
def run_server(host: str = "127.0.0.1", port: int = 4280, *, wandering_monsters: bool = True) -> None:
    """Start server in a regular function (suitable for multiprocessing target)."""
    asyncio.run(amain(host, port, wandering_monsters=wandering_monsters))
...
def main() -> None:
    ...
    run_server(args.host, args.port, wandering_monsters=not args.no_wander)
