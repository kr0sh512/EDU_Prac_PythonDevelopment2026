...
class Dungeon:
    ...
    def _monster_positions(self) -> list[tuple[int, int, Monster]]:
        out: list[tuple[int, int, Monster]] = []
    ...
    def _movable_monster_tiles(self) -> list[tuple[int, int, Monster]]:
        """Monsters that have at least one neighboring non-monster cell."""
        out: list[tuple[int, int, Monster]] = []
        for x, y, mon in self._monster_positions():
            for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                nx, ny = self._shift((x, y), dx, dy)
                if not isinstance(self[nx, ny], Monster):
                    out.append((x, y, mon))
                    break
        return out

    def wander_random_monster(
        self,
    ) -> tuple[str, str, tuple[int, int]] | None:
        """Move one random monster one step; return name, direction word, new cell."""
        candidates = self._movable_monster_tiles()
        if not candidates:
            return None
        directions = [
            (1, 0, "right"),
        ...
        for _ in range(100_000):
            x, y, mon = random.choice(candidates)
            dx, dy, dname = random.choice(directions)
            nx, ny = self._shift((x, y), dx, dy)
            if isinstance(self[nx, ny], Monster):
                continue
            self[x, y] = CellEvent()
            self[nx, ny] = mon
            return (mon.name, dname, (nx, ny))
        return None
